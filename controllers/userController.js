...
export const addUser = async (req, res) => {
  try {
    ...
    if (existingUser) {
     ...
     const userPhone = await User.findOne({ userPhone: req.validatedData.userPhone });
    if (userPhone) {
      return res.status(400).send("User with this phone number already exists");
    }
    const customerPhone = await Customers.findOne({ phone: req.validatedData.userPhone });
    if (customerPhone) {
      return res.status(400).send("Phone number already associated with a customer");
    }
    // Hash password
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(req.validatedData.userPassword, salt);
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (req.validatedData.userPassword) {
      const salt = await bcrypt.genSalt(10);
      updateData.userPassword = await bcrypt.hash(req.validatedData.userPassword, salt);
    }
